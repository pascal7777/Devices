...
app.post('/products/:id/expressions/',   upload.array('image'), isLoggedIn, catchAsync(async(req,res) => {
    const { id } = req.params;
    const product = await Product.findById(id);
    const {supplier, supCountry, gpuRef, quotation, manufacturer, brandName, mfAddress, mfCountry, deviceModel, catnr, iso9001, iso13485, number9001, number13485, iso9001validTo, iso13485validTo, iso9001onLine, iso13485onLine, iaf9001, iaf13485, body9001, body13485, DOC, DOCType, DOCSign, DOCMdClass, DOCIvdClass, DOCEq, CEcert, numberCE, CEvalidTo, CEonLine, CEcertEq, nBody, imageLabel, otherDoc, status, condition} = req.body;
    const expression = new Expression ({supplier, supCountry, gpuRef, quotation, manufacturer, brandName,  mfAddress, mfCountry, deviceModel, catnr, iso9001, iso13485,  number9001, number13485, iso9001validTo, iso13485validTo, iso9001onLine, iso13485onLine, iaf9001, iaf13485, body9001, body13485, DOC, DOCType, DOCSign, DOCMdClass,  DOCIvdClass, DOCEq, CEcert, numberCE,  CEvalidTo, CEonLine, CEcertEq, nBody, imageLabel, otherDoc, status, condition});
